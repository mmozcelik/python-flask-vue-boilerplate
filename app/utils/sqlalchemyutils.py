...
class SignallingSession(SessionBase):
    ...
    def __init__(self, db, autocommit=False, autoflush=True, **options):
        ...
        track_modifications = config.get('SQLALCHEMY_TRACK_MODIFICATIONS')
...
def _record_queries():
    rq = config.get('SQLALCHEMY_RECORD_QUERIES')
    if rq is not None:
        return rq
    ...
    def get_engine(self):
        with self._lock:
            ...
            if (uri, echo) == self._connected_for:
                return self._engine
            info = make_url(uri)
            options = {'convert_unicode': True, 'isolation_level': 'READ UNCOMMITTED'}
...
class SQLAlchemy(object):
    ...
    def apply_driver_hacks(self, info, options):
        ...
        if info.drivername.startswith('mysql'):
            ...
            #     options.setdefault('pool_size', 1)
            ...
            if not detected_in_memory:
                info.database = os.path.join(config.get('APPLICATION_PATH'), info.database)

        unu = config.get('SQLALCHEMY_NATIVE_UNICODE')
        if unu is None:
    ...
    def get_binds(self):
        ...
        """
        binds = [None] + list(config.get('SQLALCHEMY_BINDS') or ())
        retval = {}
        for bind in binds:
    ...
    def __repr__(self):
        return '<%s engine=%r>' % (
            ...
            self.engine.url if self.engine else None
        )
